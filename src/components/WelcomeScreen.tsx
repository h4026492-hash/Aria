...
import { companions, getCompanionsByGender } from '@/data/companions';
import { CompanionCharacter } from '@/types';

interface WelcomeScreenProps {
  onComplete: (name: string, companion: CompanionCharacter) => void;
}
...
export function WelcomeScreen({ onComplete }: WelcomeScreenProps) {
  ...
  const [selectedCompanion, setSelectedCompanion] = useState<CompanionCharacter | null>(null);

  const filteredCompanions = getCompanionsByGender(selectedGender);
            ...
            <div className="text-center space-y-6 animate-fadeIn">
              ...
              <div>
                <h1 className="text-4xl font-bold text-white mb-2">
                  Welcome to <span className="bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent">ARIA</span>
                </h1>
                <p className="text-white/70 text-lg">
                  Your AI companion who talks with you face-to-face
                </p>
              </div>

              <div className="bg-white/5 rounded-2xl p-4 text-left space-y-2">
                <div className="flex items-center gap-2 text-white/80 text-sm">
                  <span className="text-green-400">✓</span> Real video conversations
                </div>
                <div className="flex items-center gap-2 text-white/80 text-sm">
                ...
                <div className="flex items-center gap-2 text-white/80 text-sm">
                  <span className="text-green-400">✓</span> Always available for you
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 mt-6">
                ...
                >
                  <div className="w-20 h-20 mx-auto rounded-full overflow-hidden border-2 border-white/30 mb-3">
                    <img 
                      src={companions.find(c => c.gender === 'female')?.images.neutral}
                ...
                >
                  <div className="w-20 h-20 mx-auto rounded-full overflow-hidden border-2 border-white/30 mb-3">
                    <img 
                      src={companions.find(c => c.gender === 'male')?.images.neutral}
                      alt="Male companion"
                      className="w-full h-full object-cover"
                    />
                  </div>
