...
import { getSmartAIResponse, getSmartGreeting } from '@/services/groqService';
import { cn } from '@/utils/cn';

interface FreeVideoCallScreenProps {
  profile: UserProfile;
...
export function FreeVideoCallScreen({ 
  profile, 
  companion, 
  onEndCall, 
  onSettings 
}: FreeVideoCallScreenProps) {
  ...
  useEffect(() => {
    if (!hasGreeted) {
      const initGreeting = async () => {
        ...
        try {
          const welcomeText = await getSmartGreeting(
          ...
          setAvatarState('talking');
          speak(fallback, companion.gender, () => setAvatarState('idle'));
        }
  ...
  // Update avatar state when listening
  useEffect(() => {
    if (isListening) {
  ...
  // Update avatar state when speaking
  useEffect(() => {
    if (isSpeaking) {
      setAvatarState('talking');
    }
  ...
  }, [profile, companion, speak, stopSpeaking, isProcessing]);
  ...
  return (
    <div className="fixed inset-0 bg-gradient-to-br from-slate-950 via-purple-950 to-slate-950 flex flex-col overflow-hidden">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs bg-green-500/20 text-green-400 px-2 py-0.5 rounded-full">AI Powered</span>
            </div>
          </div>
        </div>
      ...
      {/* Main Video Area */}
      <div className="flex-1 flex items-center justify-center relative pt-20 pb-44">
        <VideoAvatar
          companion={companion}
          state={avatarState}
        ...
        {showSubtitles && currentAIText && (
          <div className="absolute bottom-48 left-4 right-4 z-20 pointer-events-none">
