...
export async function sendAudioToSadTalker(file: File, serverUrl: string): Promise<{ videoUrl: string } | { error: string }> {
  try {
...
export async function synthesizeTextToSadTalker(text: string, serverUrl: string, voice?: string): Promise<{ videoUrl: string } | { error: string }> {
  try {
